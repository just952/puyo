...
|                     | **락 딜레이 (Lock Delay) - Tsu 규칙**                                 | **완료** | 0.3초 딜레이, 이동/회전 15회 제한, 초과 시 즉시 잠금                 |
...
|                     | **입력 버그 수정 + 반시계방향 회전 지원 (v0.1.30)**                   | **완료** | DesktopInputHandler update() 순서 수정(엣지 감지), Z키/롱프레스 반시계방향, 벽킥 포함 |
...
| 데스크톱 런처       | LWJGL3 백엔드, 1600x960 가로 화면                                     | 완료     | DesktopLauncher                                                      |
...
| **반칸 단위 낙하 구현** | **Puyo.inMiddle 토글로 모든 낙하 경로 자동 부드러운 이동**           | **완료** | v0.1.24 (2026-08-22), Puyo/Board/PlayScreen 3파일만 수정, 아키텍처 최소 변경 |
| **버그 수정**      | **부유 뿌요 공중 정지 버그 수정 (열 단위 강체→개별 독립 낙하)**       | **완료** | v0.1.25 (2026-08-23), updateFallingAnimation 개별 충돌 체크로 변경    |
| **코드 중복 제거** | **canSinglePuyoFallDuringFallingAnimation() 분리, updateFallingAnimation 리팩토링** | **완료** | v0.1.25 (2026-08-23), 중복 충돌 체크 로직 공통화, ~25줄 감소         |
| **메서드명 명확화** | **canFall() → canPuyoPairFall() (private)**                          | **완료** | v0.1.25 (2026-08-23), 외부 미사용 메서드 private화 및 이름 명확화     |
| **헤드리스 테스트 추가** | **FallingAnimationTest.java 로그 기반 회귀 방지 테스트**             | **완료** | v0.1.25 (2026-08-23), 로그 보드 상태 재현 검증                        |
| **DAS/ARR 전면 개편** | **프레임→초 단위 전환, 좌우/소프트드랍 분리, 설정 외부화, TouchController 적용** | **완료** | v0.1.26 (2026-08-23), ConfigManager 연동, GameWorld 스타일 통일       |
| **착지 상태 재정의 + 바운스 애니메이션** | **Puyo.State 6단계(MOVABLE/FALLING/SETTLING/PENDING/POPPING/PLACED), SETTLING 바운스, 수직/수평 쌍 구분 착지** | **완료** | v0.1.31 (2026-09-05), LockDelay 0.3초, isLockWaiting() 헬퍼 추가       |
| **통합 이펙트 애니메이션** | **SEPARATION/FALLING + SETTLING 바운스 → PUYO_EFFECT_ANIMATION 단일 페이즈** | **완료** | v0.1.31 (2026-09-05), handlePuyoEffectAnimation, 중복 로직 제거       |
| **수직 쌍 착지 처리** | **Rotation 0/2(수직)일 때 양쪽 뿌요 모두 SETTLING, 1/3(수평)일 때 개별 착지 체크** | **완료** | v0.1.31 (2026-09-05), 원작 Tsu 규칙 준수                              |
...
| P1       | UI         | 연쇄/콤보 UI 표시 개선 (연쇄 카운트, 팝업 이펙트, 점수 애니메이션) | 중간   |
| P1       | UI         | 게임 오버/리트라이 플로우 (게임 오버 감지, 리트라이/메뉴 버튼) | 중간   |
...
| v0.1.31  | **2026-09-05** | **착지 상태 재정의 + 바운스 애니메이션 + 통합 이펙트 (Puyo.State 6단계, SETTLING/PENDING, PUYO_EFFECT_ANIMATION)** | **완료** |
...
| 날짜       | 버전  | 주요 변경                                                                                                                             |
| ---------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-05 | 0.1.31 | **착지 상태 재정의 + 바운스 애니메이션 + 통합 이펙트: Puyo.State 6단계, SETTLING 0.35초 바운스, 수직/수평 착지 구분, PUYO_EFFECT_ANIMATION 단일 페이즈** |
| 2026-08-31 | 0.1.30 | **입력 버그 수정 + 반시계방향 회전: DesktopInputHandler 엣지 감지 복구, Z키/롱프레스 반시계방향, 벽킥 포함**                            |
| 2026-08-23 | 0.1.26 | **DAS/ARR 전면 개편: 프레임→초 단위 전환, 좌우/소프트드랍 분리, 설정 외부화, TouchController 적용**                                 |
| 2026-08-23 | 0.1.25 | **부유 뿌요 공중 정지 버그 수정 + 코드 중복 제거 리팩토링 + 헤드리스 회귀 방지 테스트**                                              |
| 2026-08-22 | 0.1.24 | **반칸 단위 부드러운 낙하 구현 (Puyo.inMiddle 토글 방식, 3파일 최소 수정)**                                                            |
...
> **다음 액션**: **PC에서 헤드리스 테스트 및 데스크톱 실행 검증 후 커밋/푸시 (v0.1.31 릴리즈 준비)**