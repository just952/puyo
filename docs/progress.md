...
| 게임플레이 구현              | PlayScreen 실제 게임플레이 로직 연결                        | **100%** | GameWorld 연동, 뿌요 낙하/회전/매칭/연쇄 렌더링 (2026-08-06 완료) |
| **뷰포트/레이아웃 리팩토링** | **가로 고정 1600×960 + 모드별 레이아웃 (싱글/대전/메뉴)**   | **100%** | **GameViewport 전면 재작성, 단일 가로 해상도 (2026-08-06 완료)**  |
| **터치 컨트롤러 구현**       | **4버튼 레이아웃 (좌/우 이동, 회전, 드롭+더블탭 하드드롭)** | **100%** | **TouchController + InputHandler 통합 (2026-08-06 완료)**         |
| **모바일 가로 고정 적용**    | **AndroidManifest landscape + 몰입 모드**                   | **100%** | **이미 적용됨 (2026-08-06 확인)**                                 |
| **데스크톱 런처 가로 고정**  | **DesktopLauncher 1600×960 창 크기, 리사이즈 시 비율 유지** | **100%** | **(2026-08-06 완료)**                                             |
...
> **다음 액션**: **P1-6 터치/키보드 입력 완전 구현** (드래그 이동, 탭 회전, 하드 드롭, 터치 드래그) 및 **P1-7 뿌요 렌더링 SpriteBatch+아틀라스 전환**
