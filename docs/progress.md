...
| **뷰포트/레이아웃 리팩토링** | **가로 고정 1600×960 + 모드별 레이아웃 (싱글/대전/메뉴)** | **대기중** | GameViewport 전면 재작성, 단일 가로 해상도 |
| **터치 컨트롤러 구현** | **4버튼 레이아웃 (좌/우 이동, 회전, 드롭+더블탭 하드드롭)** | **대기중** | TouchController + InputHandler 통합 |
| **모바일 가로 고정 적용** | **AndroidManifest landscape + 몰입 모드** | **대기중** | 세로/가로 전환 로직 제거, 단순화 |
